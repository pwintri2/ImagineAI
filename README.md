...
**Director mode (up to 3 minutes)** — the 🎭 Director model plans your idea as consecutive scenes (Gemini writes the scene prompts and tags each one `safe`/`mild`/`free` when a key is configured; a keyword heuristic acts as a floor either way). Each scene is routed to an engine by that tag: **free/artistic scenes go to Wan** (Atlas' cloud Wan 2.7 first for speed; local Wan 2.2 — which has no moderation at all — as the fallback), **mild ones to Grok**, and **safe ones to Veo** (best quality, strictest filter). Scenes chain from each other's last frame — even across engines — and if a scene is refused, the Director automatically retries unchained and then falls through to the next engine in that tier. Everything is stitched into one crossfaded film with audio, tunable via `IMAGINEAI_DIRECTOR_MAX_SECONDS` and `IMAGINEAI_DIRECTOR_PLANNER_MODEL`.
